...
while (loop<MAX_LOOP) {
  loop+=1;
   ...
   if(k_mutex_ceiling_floor_lock(&my_mutex2,K_FOREVER)==0) {
    ...
    printf("thread 1 failed to lock my_mutex\n");
...
int main(void)
{
	//k_sleep(K_SECONDS(2));
  	...
  	k_mutex_ceiling_floor_init(&my_mutex2, 99, 0);
    k_mutex_ceiling_floor_init(&my_mutex, 99, 0);
...
// printf("final value of mutex_valuee is %d\n", mutex_valuee);
 assert(mutex_valuee==MAX_LOOP*5);
